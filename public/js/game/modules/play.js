import { BGMManager } from "/js/game/modules/sound.js"
import { Character } from "/js/game/modules/character.js"
import { Delay, Intro, Ending, GameOver, LogManager } from "/js/game/modules/exts.js"
import { GetSource } from "/js/game/modules/getSource.js"
...
export const play = async (isFirstTime, getUrl) => {
    if (isFirstTime) {
        let source = await GetSource(getUrl);
    ...
    avatarImg.src = `${scene.contexts[context].avatar}`;
    ...
    novelBox.addEventListener('click', async function() {
        ...
        if (context > scene.contexts.length - 1) {
            ...
            if (scene.battle) {
                ...
                bAvatarImg.src = `${scene.battleInfo.avatar}`;
                bBodyImg.src =  `${scene.battleInfo.enemy}`;
                ...
                if (scene.bgm) {
                    ...
                    setTimeout(() => {
                        ...
                        if (scene.bgm.time) BGMManager.Play(scene.bgm, scene.bgm.time||null);
                        else BGMManager.Play(scene.bgm);
                    }, 1000);
                }
                playerCharacter = new Character(scene.battleInfo.playerAtt.name, scene.battleInfo.playerAtt.hp, scene.battleInfo.playerAtt.atk, scene.battleInfo.playerAtt.def, pHpBar, pHit);
                enemyCharacter = new Character(scene.battleInfo.enemyAtt.name, scene.battleInfo.enemyAtt.hp, scene.battleInfo.enemyAtt.atk, scene.battleInfo.enemyAtt.def, eHpBar, eHit);
                playerAtkTooltip.innerText = `Tấn công: Gây sát thương trong khoảng ${Math.floor(scene.battleInfo.playerAtt.atk-(scene.battleInfo.playerAtt.atk*0.1))}-${Math.floor(scene.battleInfo.playerAtt.atk+(scene.battleInfo.playerAtt.atk*0.1))}`;
            ...
            if (isTyping) {
                ...
                if (context < scene.contexts.length) {
                    if (scene.contexts[context].avatar !== scene.contexts[context-1].avatar) avatarImg.src = `${scene.contexts[context].avatar}`;
                    if (scene.contexts[context].body) {
                        bodyImg.src =  `${scene.contexts[context].body}`;
                        bodyDiv.style.opacity = "1";
                    } else {
